...
> 插件 `ai-auto-blog-publish` v1.1.0 · 伴生服务 FastAPI（127.0.0.1:8080）
...
- **调度层**（`backend/`）：Crontab 两阶段调度（08:00 生成任务/预选题 → 20:00 复盘 → 20:30 发布），FastAPI 后台 + 备用选题池
- **AI 生成层**（`backend/agents/`）：7 智能体流水线，DeepSeek 驱动，真实数据采集注入（新浪/腾讯/baostock + Tavily 联网搜索），禁止编造行情数字
- **发布层**（`wp-plugin/`）：接收成品文章，SimHash 指纹查重（与 Python 侧算法逐字节一致），自动建文、分类映射、打标、配图、定时发布；后台提供备选题池、计划任务、日志、AI 工具箱
