...
* 摘要策略调整（翁老规则）：移除「正文前 110 字自动截取」兜底——AI 未提供摘要时 post_excerpt 留空，
  不做硬截取充数；前台由主题兼容层回退第三方插件摘要或直接不显示
* AI 工具箱列表「有无摘要」已含第三方插件兼容：post_excerpt 为空时回退读 _xhai_excerpt 再判定（v1.5.38 起）
...
= 1.5.38 =
* 工具箱兼容第三方插件字段：摘要 `_xhai_excerpt`、话题 `xhai_thread`/`xhai_postparent`
