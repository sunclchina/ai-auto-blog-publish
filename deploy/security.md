...
- uvicorn 固定 `--host 127.0.0.1 --port 8080`（`ablog.service` 已写死）。
- **严禁**改为 `0.0.0.0`；一旦绑定全网卡，8080 即对公网开放。
...
WP 插件与 Python 服务同机，经 `http://127.0.0.1:8080` 调用。若未来 WP 迁到别的机器，应改用 SSH 隧道或内网 VLAN，不要直接开公网端口。
